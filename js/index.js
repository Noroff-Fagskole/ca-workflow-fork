...
async function allPosts() {
  try {
    const response = await fetch(
      `${ALL_POSTS_URL}/?_author=true&_comments=true&limit=10`,
      {
        method: "GET",
        headers: {
          Authorization: `Bearer ${getToken()}`,
...
function listPosts(data) {
  ...
  for (let post of data) {
    ...
    onePost = `<div class="bg-white drop-shadow-md rounded-lg flex flex-col pb-4 w-96 h-fit gap-4">
...
async function allProfiles(value) {
  try {
    const response = await fetch(
      `${ALL_PROFILES_URL}?_followers=true&_following=true&_posts=true&limit=50`,
      {
        method: "GET",
        headers: {
          Authorization: `Bearer ${getToken()}`,
  ...
  for (let profile of data) {
    ...
    if (profile.followers) {
      ...
      if (amountOfFollowers === 1) {
        ...
        followersHTML = `<span class="font-semibold text-normal">${amountOfFollowers}</span> Followers`;
      }
    }
    ...
    if (profile.posts) {
      ...
      if (amountOfPosts === 1) {
        ...
        postsHTML = `<p><span class="font-semibold text-normal">${amountOfPosts}</span> Posts</p>`;
      }
    }
    ...
    oneProfile = `<a href="profile.html?id=${name}">
